  ...
  async openFile(filePath: string): Promise<void> {
    try {
      ...
      if (ext === 'md' || ext === 'txt') {
        ...
        useEditorStore.getState().addTab(filePath, filePath.split('/').pop() || '未命名', content, false, false, lastModifiedTime);
      } else if (ext === 'html') {
        // ⚠️ HTML 文件：直接读取并保持格式
        const content = await invoke<string>('read_file_content', { path: filePath });
        useEditorStore.getState().addTab(filePath, filePath.split('/').pop() || '未命名', content, false, false, lastModifiedTime);
        ...
        try {
          ...
          useEditorStore.getState().addTab(filePath, filePath.split('/').pop() || '未命名', `<p>${content}</p>`, false, false, lastModifiedTime);
        } catch (error) {
          ...
          useEditorStore.getState().addTab(filePath, fileName, placeholder, true, false, lastModifiedTime); // 只读模式
        }
      } else if (ext === 'pdf') {
        // PDF 文件：使用 iframe 预览
        useEditorStore.getState().addTab(filePath, filePath.split('/').pop() || '未命名', '', false, false, lastModifiedTime);
