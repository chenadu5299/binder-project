...
- 文档属性：`旧体系参考`
- 主责模块：`PROD`
- 文档职责：`Binder产品开发方案 / 参考、研究或索引文档`
- 上游约束：`CORE-C-D-01`, `CORE-C-D-02`, `CORE-C-D-03`, `CORE-C-D-04`, `PROD-C-D-01`
- 直接承接：无
...
3. **AI 三层架构**：辅助续写（幽灵文字）、局部修改（Cmd+K）、对话编辑（完整对话）
4. **文件管理**：直接操作本地文件，导入=复制，拖拽=移动
5. **核心功能**：SQLite 全文索引、局部修改、记忆库功能
...
│   │   ├── GhostText.tsx            # 层次一（辅助续写）：幽灵文字组件
│   │   ├── InlineAssistInput.tsx    # 层次二（局部修改）：输入框
│   ├── Chat/            # AI 聊天组件（层次三）
│   │   ├── ChatWindow.tsx          # 聊天窗口
...
├── hooks/               # 自定义 Hooks
│   ├── useFileTree.ts
│   ├── useAutoComplete.ts          # 层次一（辅助续写）：Hook
│   ├── useInlineAssist.ts          # 层次二（局部修改）：Hook
│   ├── useChat.ts                  # 层次三（对话编辑）：聊天 Hook
│   └── useEdgeSnap.ts
└── utils/               # 工具函数
    ├── fileUtils.ts
    └── formatUtils.ts
...
**编辑界面模式**：
- **分页式编辑界面**（DOCX 专用）：✅ 已实现，使用 tiptap-pagination-plus
  - 页面尺寸：A4 纵向 794×1123px
  - 自动分页：内容超出单页时自动创建新页面
  - 页码导航：工具栏内简化显示（◀ 当前页/总页数 ▶）
  - 编辑窗口缩放：50%-150%，滚动条固定
  - 实现说明：`R-ED-M-R-08_分页功能说明.md`；方案选型：`R-ED-M-R-07_T-DOCX编辑器分页模式优化方案.md`
...
pub struct AIConfig {
    ...
    pub autocomplete_trigger_delay: Duration, // 辅助续写触发延迟，默认 7 秒，可在用户设置中配置（5-15 秒范围）
...
**核心特性**：
- 无 UI 窗口，幽灵文字显示
- 快捷键触发：Cmd+J（macOS）/ Ctrl+J（Windows）主动触发，不再自动触发
- 在光标后方显示半透明续写内容（20-50 字符）
- 类似代码补全体验

**触发机制**：
- 用户按下快捷键 Cmd+J（macOS）或 Ctrl+J（Windows）
- 光标在文档中
...
| **M3：AI 功能完成** | Week 16 | AI 三层架构（辅助续写、局部修改、对话编辑）、记忆库功能 |
...
3. **AI 开发（1 人）**：AI 模型管理和集成、辅助续写算法、提示词优化、AI 功能开发
...
4. **AI 功能（三层架构）**
   - 层次一：辅助续写（快捷键触发，幽灵文字，可配置）
   - 层次二：局部修改（Cmd+K，独立输入框）
   - 层次三：对话编辑（标签栏、模型选择、引用内容、记忆库标签）
   - AI 图片生成
   - 记忆库功能（长文档场景）
