...
- **文档版本**：v1.1
- **创建日期**：2025年
- **最后更新**：2025年（同步 AI 功能需求文档，更新为三层架构）
...
**触发方式**：
- 用户按下快捷键 Cmd+J（macOS）或 Ctrl+J（Windows）主动触发
...
　
**交互操作**：　
- 触发续写：Cmd+J（macOS）/ Ctrl+J（Windows）
- 接受续写：Tab 键（将续写内容插入文档）
...
- **上下文提示词**（第二层）：
  ...
  - 当前选中的文本：选中内容
  - 工作区路径：工作区根目录
  - **当前编辑器状态**：包括是否可编辑、文件类型、文件大小、是否已保存
- **引用提示词**（第三层）：用户引用的内容列表，每个引用的类型、来源、完整内容
- **工具调用提示词**（第四层，仅 Agent 模式）：工具调用规范、工具使用说明、工具调用最佳实践
...
- 文件管理基础功能（文件树、打开、保存）
- DOCX 文件预览和编辑基础功能
- AI 功能三层架构（自动补全、Inline Assist、聊天窗口）
- 搜索功能（全文搜索）
- 多标签页功能
- 基础用户界面（布局、主题）
...
**MVP 1.0 目标**：
- 极致打磨文档编辑体验
- 完整 AI 能力（三层架构：自动补全、Inline Assist、聊天窗口）
- 稳定可靠的文件管理
- 优秀的用户体验
...
**快捷应用到文档（Quick Apply to Document）**：
- Chat 模式中，快速将聊天内容应用到文档的功能
- 支持插入、替换、追加、创建新文件等多种方式
...
**更新说明**：
- v1.1：根据 AI 功能需求文档，同步更新 AI 功能需求
  - 更新为三层架构
  - 完善聊天窗口功能（聊天内容引用、快捷应用到文档、多层提示词架构）
  - 更新 AI 请求管理和成本控制说明
  - 更新优先级和 MVP 范围
  - 更新术语表
