...
1. 保留未来 Discussion Build / Multi-Actor Build / 真人协作方向的历史设计信息。
2. 不代表当前版本实现路径。
...
3. 旧版讨论构建、多角色构建、真人协作设计，不再直接扩写 `R-BLD-*` 为当前主线。
4. 若未来 Discussion Build 获批重启，应以 Reference 为输入，重新建立独立 Active 主线，而不是直接复用旧文档作为现行规范。
