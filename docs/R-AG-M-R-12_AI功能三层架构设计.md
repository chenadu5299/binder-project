...
**触发条件**（快捷键按下时检查）：
- **须光标激活**（编辑器聚焦），否则 Cmd+J 无效，状态栏「保存状态处」滚动显示「指令无效」
- 光标在文档中
...
**触发条件**：
- **须光标激活**（编辑器聚焦），否则 Cmd+K 无效，状态栏「保存状态处」滚动显示「指令无效」
...
**与层次一、二的区别**：
- 层次一（辅助续写）、层次二（局部修改）：单次操作，无历史
- 层次三（对话编辑）：对话式，有完整历史记录
...
2. **状态隔离**：
   - 辅助续写：使用独立的 state（ghostText）
   - 局部修改：使用独立的 state（isOpen, inputValue）
   - 对话编辑：使用独立的 state（messages, tabs）

3. **事件隔离**：
   - 辅助续写：监听 selectionUpdate、update 事件
   - 局部修改：监听 Cmd+K 快捷键
   - 对话编辑：监听用户输入和发送事件
...
│   │   ├── InlineAssistInput.tsx      # 层次二（局部修改）：输入框组件
│   │   └── TipTapEditor.tsx           # 编辑器主组件
│   └── Chat/
│       ├── ChatWindow.tsx              # 层次三（对话编辑）：聊天窗口
│       ├── ChatTabs.tsx                # 标签栏
...
- **层次一（辅助续写）**：快捷键触发，无 UI 窗口，类似代码补全
- **层次二（局部修改）**：快捷键调出，单次操作，直接修改文本
- **层次三（对话编辑）**：完整对话，有历史，有工具调用，有标签栏，支持引用和快捷应用
...
**文档版本**：v2.0  
**创建日期**：2025年  
**最后更新**：2025年（根据 R-AG-M-R-05_AI功能需求文档.md 更新为三层架构）  
**基于**：R-AG-M-R-05_AI功能需求文档.md + R-PROD-C-R-02_Binder产品开发方案.md
