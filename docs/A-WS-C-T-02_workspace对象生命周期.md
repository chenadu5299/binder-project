...
- 文档属性：`主结构`
- 主责模块：`WS`
- 文档职责：`workspace对象生命周期 / 模型、架构与机制主控`
- 上游约束：`CORE-C-D-04`, `SYS-C-T-01`, `WS-C-T-01`, `WS-C-T-03`, `WS-M-D-01`
- 直接承接：无
...
本文中的生命周期规范不仅服务于文件系统，也服务于 Workspace 作为基础工作框架时的资源入口、会话绑定和执行承载语义。  
如果 `004/`A-WS-M-T-01_workspace架构.md`/`A-WS-M-D-01_workspace工作台协同主控文档.md`` 已经把 Workspace 定义为工作基础层，那么 `005` 必须保证这些对象在切换、恢复、执行和失效时仍保持同一口径。
...
| ResourcePartition | `Hidden -> Visible -> Focused -> Hidden` | 无固定终态 |
...
## 4.6 ResourcePartition（工作台分区对象）

状态定义：
1. `Hidden`：当前分区未展开或不在当前可见区。  
2. `Visible`：分区已展开，可浏览。  
3. `Focused`：分区当前处于用户显式操作焦点。  

转换规则：
- `Hidden -> Visible`：用户展开分区。  
- `Visible -> Focused`：用户在该分区执行搜索、选择、引用或管理操作。  
- `Focused -> Visible`：用户焦点切换走，但分区保持展开。  
- `Visible -> Hidden`：用户折叠分区。  

约束：
- `Focused` 只表示工作台入口焦点，不表示该分区内容已自动注入 AI 请求。  
- 分区状态变化不得直接改变任务目标文件。  
- 工作区分区默认可作为主展开分区，但不强制其他分区同时隐藏。  

---

## 4.7 TargetExecution（执行门禁对象）
...
6. WorkspaceSession 进入 `Switching` 时：  
所有 `ResourcePartition(Focused)` 必须降为非焦点态，防止旧 workspace 焦点串用。  

7. ResourcePartition 从 `Focused -> Visible/Hidden` 时：  
不得直接触发 TargetExecution 状态推进；执行推进必须来自显式任务链。  
...
1. 来源参考：  
- `R-WS-M-R-02_Workspace改造可落地实施方案.md`（阶段流转和执行策略）  
- `binder开发协同.md`（协同事件与状态字段）  
- `A-WS-M-D-01_workspace工作台协同主控文档.md`（工作台分区与入口焦点语义）  

2. 对齐文档：  
- `A-WS-C-T-01_workspace模型定义.md`（对象模型主定义）  
- `A-WS-M-T-01_workspace架构.md`（Workspace 作为工作底座的架构定位）  
- `R-DE-M-R-02_对话编辑-统一整合方案.md`（Diff 主链规则）  
- `R-DE-M-R-03_文档逻辑状态传递规范.md`（逻辑状态边界）  
