...
2. `对话编辑-主控设计文档.md`（主控实现口径）。
3. 本文（Diff 专项落地规范）。
4. `311_对话编辑统一方案.md`（跨模块总纲）。
...
|---|---|---|---|---|
| DE-VIS-001 | 主承接 | 4.1 文档侧规则 | 311§9.2 | 8.2 |
| DE-VIS-002 | 主承接 | 4.2 聊天侧卡片规则 | 311§9.2 | 8.2 |
| DE-EXP-001 | 主承接 | 6 失效规则 | 311§9.2 | 8.3 |
| DE-OBS-005 | 主承接 | 6.3 失败与失效隔离 | 311§9.2 | 8.3 |
| DE-OUT-001 | 协同承接 | 3.1 canonical diff 字段 | 311§9.1 | 8.1 |
| DE-OUT-002 | 协同承接 | 3.2 区间关系约束 | 311§9.2 | 8.2 |
| DE-OUT-003 | 协同承接 | 3.3 跨 Block 语义 | 311§9.2 | 8.1 |
| DE-ORI-001 | 协同承接 | 3.4 originalText 规则 | 311§9.1 | 8.2 |
| DE-EXEC-001 | 协同承接 | 5.1 单卡执行 | 311§9.1 | 8.1 |
| DE-EXEC-002 | 协同承接 | 5.2 批量执行 | 311§9.2 | 8.2 |
| DE-EXEC-003 | 协同承接 | 5.3 diff 池一致性 | 311§9.2 | 8.2 |

说明：
1. “主承接”表示该规则的展示/交互细则由 312 给出。
2. “协同承接”表示主定义在 311/313，312 提供 Diff 视角落地约束。
...
## 九、与 311/313/314 的反向链接

1. 与 `311` 对齐：主链阶段、执行门禁、上线验收。
2. 与 `313` 对齐：状态语义、`baseline/revision` 判定边界。
3. 与 `314` 对齐：提示词不得把 pending 描述为已生效。
...
1. `311_对话编辑统一方案.md`
2. `313_baseline状态协作.md`
3. `314_对话编辑提示词.md`
4. `对话编辑-统一整合方案.md`
5. `对话编辑-主控设计文档.md`
6. `跨 Block Diff 实现方案.md`
7. `Diff效果优化方案.md`
