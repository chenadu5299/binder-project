...
| 类型 | 结构 | 与 AI |
|------|------|-------|
| **文档模板** | `{ id, name, type: 'document', structure }` | 作为格式约束注入；create_document 协同 |
...
| **Skills** | `{ id, name, type: 'skill', content, scene?, requiresAuth? }` | **纯自然语言 content**，AI 直接消费；@ 引用或「应用 Skill」 |
...
**Skills 说明**：Skills 为用户创建、保存、复用的行为说明；无 steps 数组、无 tools 字段；系统 Skills 只读，用户 Skills 可编辑；依赖外部服务时 requiresAuth 触发授权引导。
...
| 模板库格式 | 工作流模板 Plan JSON 结构（id、name、source、weighted_score、steps）；Skills 自然语言 content 结构 |
