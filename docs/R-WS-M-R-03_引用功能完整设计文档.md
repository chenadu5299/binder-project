...
> 当前口径以：`A-CORE-C-D-02_产品术语边界.md`、`A-AST-M-P-01_上下文注入.md`、`A-DE-M-D-01_对话编辑统一方案.md`、`A-AST-M-T-07_Binder知识库自动检索协同规范.md`、`A-WS-M-T-05_跨文档操作.md` 为准
> 可用于：`追溯旧引用输入方案、旧优先级设计和历史降级策略`
> 不可用于：`作为当前引用协议、当前文档优先级或显式当前文件锚点规则的主依据`
> 历史说明：本文中的旧优先级、占位能力和未实现降级策略均为历史背景，不构成当前 Active 协议授权。
...
引用系统在历史设计中被视为 Binder 对话编辑（层次三）的核心输入增强层。当前 Active 口径已拆分为术语、上下文注入、知识检索边界和跨文档操作等多份 Active 文档协同定义。
