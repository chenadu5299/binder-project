...
- 文档属性：`归档参考`
- 主责模块：`AG`
- 文档职责：`Binder-AI功能优化方案-落地版-架构层-v2 / 参考、研究或索引文档`
...
| 文档 | 关系 |
|------|------|
| `对话编辑-统一整合方案（待确认版）.md`、`R-DE-M-R-13_层次三（对话编辑）提示词详细分析文档.md` | **同一条执行链**：统一整合方案与提示词主控文档负责对话编辑规则、状态、提示词收口；本文负责任务抽象与调度 |
| `提示词代码实现梳理.md` | 实现细节与 **continue 状态机**；编排层落地时需**对照**避免双套逻辑 |
| `R-DE-M-R-01_对话编辑-主控设计文档.md` | **diff_patch** 与 **Editor 层** 的底层依据 |
