...
- 文档属性：`参考`
- 主责模块：`DE`
- 文档职责：`层次二-局部修改功能设计文档 / 参考、研究或索引文档`
...
- **文档性质**：功能设计文档
- **功能层次**：层次二（局部修改）
- **关联规范**：`R-DE-M-R-07_局部修改弹窗实现说明.md`（实现规范）
...
- **灵活交互**：**支持无选区调出**（无选区时默认内容为空）；**先调出再选中**时实时同步选中内容
- **须光标激活**：编辑器聚焦时 Cmd+K 有效；未聚焦时无效，状态栏「保存状态处」滚动显示「指令无效」
- **直接应用结果**：反馈直接应用到选中的文本区域或光标位置
- **弹窗内多轮对话**：可多轮追问，messages 结构（role、text）；**弹窗内不做 Diff 效果**
...
- **须光标激活**：编辑器聚焦时有效；未聚焦时无效，状态栏滚动提示「指令无效」
- **支持无选区调出**：无选区时弹窗默认内容为空；有选区时为选中文本
- 在光标位置或选中文本位置显示弹窗

### 2.2 触发逻辑

- 监听 Cmd+K 快捷键，判断编辑器是否聚焦
- 获取选中的文本（有选区则为选中内容，无选区则为空）
- 显示弹窗
- **先调出再选中**：弹窗已打开时，用户选中内容 → 窗口实时显示选中内容，新选中内容进入后续处理逻辑
- 用户输入指令后执行

### 2.3 选区保持与弹窗布局定位

- **选区保持**：激活窗口后选区不取消；不自动聚焦输入框；selectionRange 存储用于 apply
- **布局**：输入框固定在上方，内容区在下方；执行前后一致，输入框顶部稳定
- **定位**：窗口始终左右居中；垂直位置**始终在选区下方**；选区下方空间不足时改为**光标上方**
...
- **弹窗内不做 Diff**：不展示修改前后对比
- **修改类**：返回修改后的文本，直接应用
- **生成类**：返回生成的内容，直接插入
- **分析类**：返回分析结果（也以可插入内容形式）
- **匹配类**：返回匹配结果
...
获取记忆库信息（P1 优先级，可选）：
  ...
  - 最多获取 3-5 条相关记忆
    ↓
调用后端 ai_inline_assist（传递 instruction、text、context、messages 等）
    ↓
显示结果（弹窗内不做 Diff，只展示可直接插入的内容）
    ↓
用户确认 → 点击应用按钮 → 应用到编辑器 → 关闭弹窗
用户取消 → 点击关闭或再次 Cmd+K → 关闭弹窗
```

### 4.2 后端工作流程（优化后）

```
接收请求（instruction, text, context, messages?, memoryItems）
    ↓
...
- **结果展示**：弹窗内不做 Diff，只展示可直接插入的内容；弹窗有应用按钮

### 4.3 状态管理

- **Hook 状态**：`useInlineAssist` 管理状态
- **状态字段**：`isVisible`（是否可见）、`instruction`（指令）、`selectedText`（选中文本）、`messages`（对话历史 `{ role, text }[]`）、`isLoading`（是否加载中）、`error`（错误信息）
- **状态更新**：响应用户操作和 AI 响应
...
**弹窗内不做 Diff 效果**：不展示修改前后对比，只回复可直接插入的内容。
...
- **命令**：`ai_inline_assist`（需增加 `messages: Option<Vec<{role, text}>>` 参数）
...
- **已决策**：弹窗内不展示修改前后对比（红删绿增）
- 弹窗只回复可直接插入的内容，有应用按钮，点击直接应用

### 11.2 多轮对话

- **messages 结构**：`{ role: "user" | "assistant", text: string }[]`
- **ai_inline_assist 入参**：需增加 `messages` 参数，后端按对话历史拼接提示词
- **无对话历史展示**：每次交互覆盖上一次回复，只展示可直接插入的内容
