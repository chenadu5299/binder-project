...
> 说明：本文是历史开发计划文档。凡涉及层次三对话编辑的提示词规则、目标文件语义、零搜索、块列表、工具后续语义，一律以 [R-DE-M-R-13_层次三（对话编辑）提示词详细分析文档.md](/Users/chenzhenqiang/Desktop/test/binder-project/docs/R-DE-M-R-13_层次三（对话编辑）提示词详细分析文档.md)、[R-DE-M-R-01_对话编辑-主控设计文档.md](/Users/chenzhenqiang/Desktop/test/binder-project/docs/R-DE-M-R-01_对话编辑-主控设计文档.md)、[对话编辑-统一整合方案（待确认版）.md](/Users/chenzhenqiang/Desktop/test/binder-project/docs/对话编辑-统一整合方案（待确认版）.md) 为准。本文中旧的 `plan_workflow`、层次三独立规划判断等描述不再作为当前依据。
...
#### 任务 3.3：层次三任务分析提示优化（P0 优先级）

**目标**：通过提示词让 AI 正确判断目标文件、定位模式和工具调用路径
...
执行流程：
1. 先识别目标任务：
   - 判断是否为当前文件编辑、非当前文件编辑、外部参考读取或普通问答
   - 判断是否具备精确定位输入

2. 再决定调用路径：
   - 当前与非当前目标文档编辑统一走对话编辑主链
   - 有精确定位输入时直接零搜索
   - 无精确定位输入时使用块列表和 `block_index`
