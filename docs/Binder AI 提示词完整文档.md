...
[续写要求]
基于以上上下文，续写接下来的内容（1-80 字符，以完整句子或语义单元结尾）：
...
- 层次三提示词专项规则以 `层次三（对话编辑）提示词详细分析文档.md` 为准
- 对话编辑业务链与协议以 `对话编辑-主控设计文档.md` 和 `对话编辑-统一整合方案（待确认版）.md` 为准
- 本章只保留总览，不再保留旧版对话编辑提示词细节
...
- 层次三不再使用 `plan_workflow`
- 对话编辑任务的系统提示必须围绕以下规则构建：
  - 以目标编辑文件为中心，而不是只围绕当前活动文件
  - 有精确坐标时直接走零搜索，不重新全文搜索
  - 无精确坐标时才使用 `[文档块列表] + block_index`
  - 多块编辑逐块调用工具
  - `rewrite_document` 只用于明确全文重写任务
  - 生成的是待确认 diff，不得描述成“已经应用到文档”
...
[目标编辑文件]
文件路径：[目标文件路径]
目标文件内容以本轮 RequestContext 的 L 为准

[定位模式]
- 若系统已提供精确坐标：直接按精确定位处理
- 若系统未提供精确坐标：使用 [文档块列表] + block_index
```

**工作逻辑**：
- 对话编辑只围绕目标文件组织上下文
- 目标文件可以是当前文件，也可以是非当前文件
- 是否当前打开不决定是否可以编辑
- 任务意图决定目标文件
...
- 精确引用：带 `startBlockId/startOffset/endBlockId/endOffset`，进入零搜索路径
- 普通引用：仅作为参考内容

层次三不再使用行号作为定位展示口径，统一使用块号、区间信息或目标文件标题。
...
- `Review the result`（若这里的 result 指已落地文档）
- 任何把 pending diff 说成最终状态的表达
...
- 简单任务直接执行
- 复杂任务由 AI 在自然语言中简要说明执行计划
- 对话编辑任务重点约束定位协议、Diff 状态与工具调用边界
- 不再通过 `plan_workflow` 生成和传递规划对象
...
- 续写长度：1-80 字符（字=字符），以完整句子或语义单元结尾
