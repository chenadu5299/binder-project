...
- 接口耦合：`AG-M-T-05`, `AST-M-D-02`, `AST-M-D-03`, `AST-M-P-01`, `WS-M-D-01`, `CBT-I-P-01`, `CBT-I-T-02`
- 汇聚影响：`TMP-M-S-01`, `TMP-M-S-02`, `TMP-M-T-01`, `CORE-C-R-01`
- 扩散检查：`TMP-M-T-01`, `TMP-M-S-01`, `TMP-M-S-02`, `AG-M-T-05`, `AST-M-P-01`, `A-CBT-I-T-02_Chat Build执行链与运行控制.md`, `WS-M-D-01`
- 使用边界：`定义模板库技术主链、对象体系、执行协同、规则映射与专项分工；不直接承担数据库定稿、UI 视觉稿与开发排期`
- 变更要求：`修改本文后，必须复核：TMP-M-D-01、TMP-M-D-02、TMP-M-T-01、TMP-M-S-01、TMP-M-S-02、AG-M-T-05、AST-M-P-01、CBT-I-T-02、WS-M-D-01`
...
| `TT-INT-*` | 与 AG / AST / WS / CBT 的接口协同规则 | 本文 |
...
| `TT-CORE-002` | 模板库只承接过程约束，不承接内容、结构或行为定义 | `TMP-M-D-02` §4/§10 | 本文 / `TMP-M-T-01` | `AST-M-P-01`, `CBT-I-T-02` |
| `TT-CORE-003` | 原始模板文档不得直接进入执行，必须先解析再编译 | `TMP-M-D-02` §6.6-§6.7 | 本文 / `TMP-M-S-01` | `AG-M-T-05`, `CBT-I-T-02` |
...
| `TT-CORE-006` | 模板不是独立执行器，执行主控始终是 Agent | `TMP-M-D-02` §7/§9 | 本文 / `TMP-M-S-02` | `AG-M-T-05`, `CBT-I-T-02` |
...
4. CBT：作为模板库在构建态下的消费方之一。

依赖模板库的模块：

1. AG 文档生成与任务执行主链。
2. WS 工作台中的模板创建、编辑、选择与状态展示。
3. Chat Build 中的大纲规划链。
...
|---|---|---|---|---|---|---|---|
| `WorkflowTemplate` | 模板资产层 | TMP 资产创建链 | WS / TMP / AG / CBT | 长期资产 | 是 | 否 | 否 |
| `WorkflowTemplateDocument` | 编辑 / 存储层 | 用户编辑链 | 解析层 | 长期资产表达层 | 是 | 否 | 否 |
| `ParsedWorkflow` | 结构解析层 | 解析链 | 编译层 | 中间产物 | 可选 | 是 | 否 |
| `CompiledWorkflow` | 流程编译层 | 编译链 | AG / CBT / 运行时映射层 | 中间产物 | 可选 | 是 | 否 |
...
### 9.4 与 CBT（Chat Build）

边界：

1. CBT 不把模板当独立执行器。
2. 模板在 Chat Build 中是规划与执行约束输入。

技术关系：

1. CBT 读取模板引用。
2. CBT 消费编译后的结构化流程表示。
3. CBT 将其吸收到 build outline，而不是直接运行模板。
...
6. `A-CBT-I-T-02_Chat Build执行链与运行控制.md`
