...
> 分页模式下，跨页长光标会露出。本方案通过「页顶光标模拟」与「上下遮罩」两种手段解决该问题。
...
| 场景 | 现象 | 解决手段 |
|------|------|----------|
| **页顶** | 光标在页首行时，原生光标可能跨页露出 | 隐藏原生光标 + 显示模拟光标 |
...
│  分页模式光标与遮罩方案                                        │
├─────────────────────────────────────────────────────────────┤
│  方案 A：页顶光标（PageTopCaretExtension）                      │
│  - 分页模式 + 光标态：隐藏原生，显示 div.rm-page-top-caret       │
...
| 页顶光标 | 独立，不依赖遮罩 |
| 页底遮罩 | 独立，不依赖页顶光标 |
| PaginationPlus 遮罩 | 页顶光标不依赖 mask 判定，仅依赖分页 DOM 存在 |
...
**原生光标隐藏**：通过 `data-caret-at-page-top` 控制
...
| 功能 | 文件 |
|------|------|
| 页顶光标扩展 | `src/components/Editor/extensions/PageTopCaretExtension.ts` |
| 页顶光标样式 | `src/index.css`（`.rm-page-top-caret`、`caretAtPageTop`） |
