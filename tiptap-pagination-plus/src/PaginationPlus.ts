...
import { deepEqualIterative, footerClickEvent, getCustomPages, getFooter, getFooterHeight, getHeader, getHeaderHeight, getHeight, headerClickEvent, updateCssVariables, computePerPageMaskBottomHeights, computeCaretMaskBottomHeight } from "./utils";
...
/** 遮罩调试：设为 true 时控制台输出遮罩状态 */
const DEBUG_CARET_MASK = false;

/** 页底遮罩重算防抖延迟（ms） */
const MASK_BOTTOM_DEBOUNCE_MS = 120;

let maskBottomDebounceId: ReturnType<typeof setTimeout> | null = null;
...
export const PaginationPlus = Extension.create<PaginationPlusOptions, PaginationPlusStorage>({
  ...
  onCreate() {
    ...
    updateCssVariables(targetNode, this.options, DEBUG_CARET_MASK);
    ...
    style.textContent = `
      ...
      .rm-caret-mask-bottom{
        bottom: 100%;
        height: var(--rm-caret-mask-bottom-height, 48px);
      }
      .rm-caret-mask-top{
        top: 100%;
        height: var(--rm-caret-mask-top-height, 48px);
      }
      .rm-with-pagination .rm-page-break:last-child .rm-caret-mask-wrapper{
        display: none;
      }
      .rm-with-pagination,
          ...
          apply: (tr, oldDeco, oldState, newState) => {
            try {
              ...
              const getNewDecoration = () => {
                updateCssVariables(editor.view.dom, this.options, DEBUG_CARET_MASK);
              ...
              // editor.view 可能尚未挂载或已销毁（如切换布局模式、卸载时）
              return oldDeco;
            }
          },
        ...
        view: (editorView: EditorView) => {
          return {
            update: (view : EditorView) => {
                ...
                // 页底遮罩：每页独立高度（方案 A 待实现，当前用 fallback），防抖
                const fallbackBottom = computeCaretMaskBottomHeight(this.options);
                if (maskBottomDebounceId) clearTimeout(maskBottomDebounceId);
                maskBottomDebounceId = setTimeout(() => {
                  maskBottomDebounceId = null;
                  requestAnimationFrame(() => {
                    try {
                      const heights = computePerPageMaskBottomHeights(view.dom, fallbackBottom);
                      const maskBottoms = view.dom.querySelectorAll<HTMLElement>(".rm-page-break .rm-caret-mask-bottom");
                      maskBottoms.forEach((mask, i) => {
                        const h = heights[i] ?? fallbackBottom;
                        mask.style.height = `${h}px`;
                      });
                    } catch (_) {
                      // 测量失败时保持 CSS 变量 fallback
                    }
                  });
                }, MASK_BOTTOM_DEBOUNCE_MS);

                return ;
                } catch {
                  // view.dom 可能不可用（编辑器未挂载或已销毁）
                }
            }, 
          }
        }
  ...
  addCommands() {
    return {
      ...
      updatePageSize: (size: PageSize) => () => {
        ...
        // 立即更新 CSS 变量，确保横版等布局切换即时生效
        if (this.editor?.view?.dom) {
          updateCssVariables(this.editor.view.dom, this.options, DEBUG_CARET_MASK);
        }
        return true;
      },
    ...
    (view) => {
      ...
      const pageBreakDefinition = (firstPage: boolean, pageHeader: HTMLElement, pageFooter: HTMLElement, headerHeight: number, footerHeight: number, pageNumber?: PageNumber) => {
        ...
        if (pageNumber != null) {
          ...
          pageNumEl.className = "rm-gap-page-number";
          pageNumEl.textContent = String(pageNumber);
          pageSpace.appendChild(pageNumEl);
        }

        const maskWrapper = document.createElement("div");
        maskWrapper.classList.add("rm-caret-mask-wrapper");
        const maskBottom = document.createElement("div");
        maskBottom.classList.add("rm-caret-mask-bottom");
        const maskTop = document.createElement("div");
        maskTop.classList.add("rm-caret-mask-top");
        maskWrapper.append(maskBottom, pageSpace, maskTop);

        pageBreak.append(pageFooter, maskWrapper, pageHeader);
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        if(headerPageNumber in pageOptions.customHeader || pageNumber in pageOptions.customFooter || pageNumber in pageOptions.customHeader) {
          ...
          fragment.appendChild(pageBreakDefinition(i === 0, __pageHeader, __pageFooter, _headerHeight, _footerHeight, pageNumber));
        }
