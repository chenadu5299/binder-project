...
impl WorkspaceDb {
    ...
    pub fn get_workflow_template(&self, template_id: &str) -> Result<Option<WorkflowTemplate>, String> {
        let conn = self.conn.lock().map_err(|e| format!("锁失败: {}", e))?;
        let workspace_str = self.workspace_path.to_string_lossy();
        let mut stmt = conn
            .prepare(
        ...
        if let Some(row) = rows.next().map_err(|e| format!("next get_workflow_template 失败: {}", e))? {
            let status: String = row.get(5).map_err(|e| format!("get status: {}", e))?;
            Ok(Some(WorkflowTemplate {
                id: row.get(0).map_err(|e| format!("get id: {}", e))?,
                workspace_path: row.get(1).map_err(|e| format!("get workspace_path: {}", e))?,
    ...
    pub fn bump_workflow_template_version(
        ...
        self.get_workflow_template(template_id)?
    ...
    pub fn get_workflow_template_document(
        ...
        if let Some(row) = rows.next().map_err(|e| format!("next get_workflow_template_document 失败: {}", e))? {
    ...
    pub fn get_parsed_workflow_cache(
        ...
        if let Some(row) = rows.next().map_err(|e| format!("next get_parsed_workflow_cache 失败: {}", e))? {
    ...
    pub fn upsert_compiled_workflow_cache(
        &self,
    ...
    pub fn get_compiled_workflow_cache(
        ...
        if let Some(row) = rows.next().map_err(|e| format!("next get_compiled_workflow_cache 失败: {}", e))? {
            let compiled_json: String = row.get(0).map_err(|e| format!("get compiled_json: {}", e))?;
    ...
    pub fn upsert_template_binding(
        &self,
    ...
    pub fn upsert_runtime_workflow_plan(
        &self,
    ...
    pub fn get_runtime_workflow_plan(
        ...
        if let Some(row) = rows.next().map_err(|e| format!("next get_runtime_workflow_plan 失败: {}", e))? {
    ...
    pub fn get_workflow_execution_runtime(
        ...
        {
            let runtime_json: String = row.get(0).map_err(|e| format!("get execution_runtime_json: {}", e))?;
    ...
    pub fn get_agent_tasks_by_chat_tab(&self, chat_tab_id: &str) -> Result<Vec<AgentTaskRow>, String> {
        let conn = self.conn.lock().map_err(|e| format!("锁失败: {}", e))?;
    ...
    pub fn get_agent_artifacts_by_task(&self, task_id: &str) -> Result<Vec<AgentArtifactRow>, String> {
        let conn = self.conn.lock().map_err(|e| format!("锁失败: {}", e))?;
    ...
    pub fn insert_timeline_node_with_payload(
        ...
        tx.commit().map_err(|e| format!("提交时间轴事务失败: {}", e))?;
        Ok(())
    }
    ...
    fn trim_timeline_nodes_in_tx(
        ...
        for (node_id, payload_id) in stale {
            tx.execute("DELETE FROM timeline_nodes WHERE node_id = ?1", params![node_id])
    ...
    pub fn get_timeline_node(&self, node_id: &str) -> Result<Option<TimelineNodeRecord>, String> {
        ...
        if let Some(row) = rows.next().map_err(|e| format!("next 失败: {}", e))? {
            let impact_scope_json: String = row.get(5).map_err(|e| format!("get impact_scope_json: {}", e))?;
            ...
            Ok(Some(TimelineNodeRecord {
                ...
                operation_type: row.get(3).map_err(|e| format!("get operation_type: {}", e))?,
                summary: row.get(4).map_err(|e| format!("get summary: {}", e))?,
                impact_scope,
                actor: row.get(6).map_err(|e| format!("get actor: {}", e))?,
                restorable: row.get::<_, i64>(7).map_err(|e| format!("get restorable: {}", e))? != 0,
                restore_payload_id: row.get(8).map_err(|e| format!("get restore_payload_id: {}", e))?,
