...
pub async fn open_file_with_cache(
  ...
  let full_path = resolve_target_file_under_workspace(&workspace_path, &file_path)
    .map_err(|e| gate_error("TARGET_FILE_RESOLVE_FAILED", &e, &gates))?;
...
pub async fn open_docx_with_cache(
  ...
  let content = match &cached {
    ...
    _ => {
      ...
      let (html, hash) = canonical_html_for_workspace_cache(&raw);
      db.upsert_file_cache(&file_path, "docx", Some(&html), Some(hash.as_str()), mtime)?;
      eprintln!(
...
pub async fn accept_file_diffs(
  ...
  let (body_to_store, content_hash_opt) = if should_run_workspace_canonical_pipeline(&file_type) {
  ...
  if file_type == "docx" {
    ...
    pandoc
      .convert_html_to_docx(&body_to_store, &full_path)
  ...
  // 记忆内容提取（文本文件，60s 节流，fire-and-forget）
  {
    ...
    if is_text && !html_content.trim().is_empty() {
      ...
      let provider_opt = {
        ...
        guard.and_then(|g| {
          g.get_provider("deepseek")
            .or_else(|| g.get_provider("openai"))
        })
      };
      if let Some(provider) = provider_opt {
        ...
        tokio::spawn(async move {
          ...
          crate::services::memory_service::memory_generation_task_content(provider, ws, fp, html)
            .await;
        });
      }
    }
...
fn relative_path_under_workspace(workspace_path: &str, file_path: &str) -> Option<String> {
  ...
  // 已是工作区相对路径（无前导盘符 / 根）
  let looks_absolute =
    f.starts_with('/') || f.starts_with("//") || f.contains(":/") || f.contains(":\\");
  if !looks_absolute {
    return Some(
...
pub async fn get_file_dependencies(
  workspace_path: String,
) -> Result<Vec<FileDependencyDto>, String> {
  let db = WorkspaceDb::new(Path::new(&workspace_path))?;
