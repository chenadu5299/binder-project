  ...
  match payload.payload_kind.as_str() {
      ...
      for file in files {
        ...
        let current = if full_path.exists() {
          general_purpose::STANDARD
            .encode(std::fs::read(&full_path).map_err(|e| format!("读取当前文件失败: {}", e))?)
        } else {
          String::new()
        };
        if current != desired {
    ...
    "resource_structure" => {
      ...
      for relative in &node.impact_scope {
        ...
        capture_resource_state_recursive(
          workspace_root,
          &full_path,
          &mut current_entries,
          &mut seen,
  ...
  match payload.payload_kind.as_str() {
    "file_content" => {
      ...
      for file in files {
        ...
        if file_type == "docx" {
          ...
          std::fs::write(&full_path, content).map_err(|e| format!("写入还原文件失败: {}", e))?;
        }
    ...
    "resource_structure" => {
      ...
      for entry in present_files {
        ...
        std::fs::write(&full_path, bytes).map_err(|e| format!("写入资源文件失败: {}", e))?;
      }
