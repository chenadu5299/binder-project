...
use crate::services::memory_service::{
    MemoryService, MemorySearchScope, MemorySearchResponse, SearchMemoriesParams,
};
use std::path::Path;
...
pub async fn on_tab_deleted_cmd(
    ...
    let provider = {
        ...
        guard.get_provider("deepseek").or_else(|| guard.get_provider("openai"))
    };
...
#[tauri::command]
pub async fn expire_memory_item(
    memory_id: String,
    workspace_path: String,
) -> Result<(), String> {
    ...
    let service = MemoryService::new(Path::new(&workspace_path))
        .map_err(|e| format!("MemoryService 初始化失败: {}", e))?;
    service.expire_item(&memory_id).await.map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn expire_memory_layer(
    layer: String,
    workspace_path: String,
) -> Result<u64, String> {
    ...
    let service = MemoryService::new(Path::new(&workspace_path))
        .map_err(|e| format!("MemoryService 初始化失败: {}", e))?;
    service.expire_layer(&layer).await.map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn get_memory_user_data() -> Result<crate::services::memory_service::UserMemoryInfo, String> {
    ...
    let user_db_path = crate::services::memory_service::user_memory_db_path()
        ...
        .to_string_lossy()
        .to_string();
    Ok(crate::services::memory_service::UserMemoryInfo { user_id, user_db_path })
}

// ── P1：启动维护 ──────────────────────────────────────────────────────────────

#[tauri::command]
pub async fn startup_memory_maintenance(
    workspace_path: String,
) -> Result<(), String> {
