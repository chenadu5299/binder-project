use crate::services::knowledge::{
    KnowledgeDeleteRequest, KnowledgeIngestRequest, KnowledgeMoveRequest, KnowledgeQueryRequest,
    KnowledgeQueryResponse, KnowledgeEntryListResponse, KnowledgePolicyUpdateRequest,
...
pub async fn query_knowledge_base(
    ...
    let service = KnowledgeService::new(Path::new(&workspace_path))?;
    service.query_knowledge_base(request).map_err(|e| e.to_string())
}
...
pub async fn update_knowledge_entry_policy(
    ...
    let service = KnowledgeService::new(Path::new(&workspace_path))?;
    service.update_entry_policy(request).map_err(|e| e.to_string())
}
...
pub async fn list_knowledge_entries(
    ...
    let service = KnowledgeService::new(Path::new(&workspace_path))?;
    service
        .list_knowledge_entries(knowledge_base_id.as_deref(), query.as_deref(), limit.unwrap_or(50))
        .map_err(|e| e.to_string())
}
