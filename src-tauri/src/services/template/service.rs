...
use super::types::{
  ...
  CompiledWorkflowStep, ExecutionContext, ExecutionStage, ExecutionState, ParsedWorkflow,
  ParsedWorkflowPhase, ParsedWorkflowStep, RuntimeWorkflowPlan, RuntimeWorkflowPlanStep,
  StepExecutionStatus, StepState, TemplateBinding, WorkflowDiagnostic, WorkflowDiagnosticKind,
...
impl TemplateService {
  ...
  pub fn apply_tool_execution_feedback(
    ...
    if success {
      ...
      let compiled =
        self.compile_workflow_template(workspace_path, &runtime.context.template_id)?;
      ...
      if next_index < runtime.step_states.len() {
        ...
        runtime.execution_state.waiting_for_user = false;
        runtime.execution_state.updated_at = now;
        runtime.execution_state.reason =
          Some(format!("step_advanced_after_tool_success:{}", tool_name));
        ...
        runtime.execution_state.waiting_for_user = false;
        runtime.execution_state.updated_at = now;
        runtime.execution_state.reason = Some(format!(
          "workflow_completed_after_tool_success:{}",
          tool_name
        ));
      }
    } else {
      ...
      self.append_runtime_diagnostic(
        ...
        WorkflowDiagnostic {
          ...
          message: format!(
            "步骤“{}”在工具“{}”执行后进入失败分支：{}。",
            current_step.name, tool_name, failure_reason
          ),
          phase_name: Some(current_step.phase_name.clone()),
          step_name: Some(current_step.name.clone()),
        },
  ...
  fn default_execution_stage_for_compiled(&self, compiled: &CompiledWorkflow) -> ExecutionStage {
  ...
  fn parse_document(&self, template_id: &str, content: &str, updated_at: i64) -> ParsedWorkflow {
    ...
    for raw_line in content.lines() {
      ...
      if let Some(step) = current_step.as_mut() {
        ...
        if let Some(value) = Self::parse_field_value(line, &["constraint", "constraints", "约束"])
        {
          ...
          step.constraint = if constraint.is_empty() {
            None
          } else {
            Some(constraint)
          };
...
mod tests {
  ...
  use crate::services::template::types::{
    CompiledWorkflowRiskLevel, CompiledWorkflowStatus, StepExecutionStatus, WorkflowTemplate,
...
"#;
  ...
  fn tool_success_feedback_advances_runtime_step() {
    ...
    assert!(matches!(
      runtime.step_states[0].status,
      StepExecutionStatus::Completed
    ));
    assert!(matches!(
      runtime.step_states[1].status,
      StepExecutionStatus::Running
    ));
  }
