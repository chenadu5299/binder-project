...
static DATA_BLOCK_ID_ATTR: Lazy<Regex> = Lazy::new(|| {
  Regex::new(r#"(?i)data-block-id\s*=\s*["']([^"']*)["']"#).expect("DATA_BLOCK_ID_ATTR regex")
});
...
pub struct Diff {
  ...
  /// strip 标签后、旧版纯文本中的半开区间 [start, end)。仅用于合并判断：避免把多处独立替换拼成原文不存在的子串（如 `world`+`world`→`worldworld`），导致 `diff_to_canonical` 无法定位。
  #[serde(default, skip_serializing)]
  pub old_text_char_range: Option<(usize, usize)>,
  /// 与 `find_block_range_for_text` 对齐：全文拼接文本中第几处子串匹配（0-based）。`None` 等价于 0。
  #[serde(default, skip_serializing)]
...
impl DiffService {
  ...
  /// 与 [`Self::build_position_map`] 相同的可见字符序列，并为每个字符记录当前所在 `data-block-id`（内层优先）。
  /// 用于将 `calculate_diff` 产出的 `old_text_char_range` 精确映射到 canonical 的 block 偏移（多处置换同一子串时避免总落在首次匹配）。
  fn build_stripped_text_with_block_ids(html: &str) -> (String, Vec<Option<String>>) {
    let mut text_content = String::new();
    let mut char_block_ids: Vec<Option<String>> = Vec::new();
    let mut in_tag = false;
    let mut in_quotes = false;
    let mut quote_char = '\0';
    let mut tag_buf = String::new();
    let mut el_stack: Vec<Option<String>> = Vec::new();
    ...
    for ch in html.chars() {
      if in_tag {
        ...
        if !in_quotes && (ch == '\'' || ch == '"') {
          ...
          Self::apply_open_close_tag(&tag_buf, &mut el_stack);
          tag_buf.clear();
          in_tag = false;
        }
      } else if ch == '<' {
        ...
        let bid = el_stack.iter().rev().find_map(|x| x.as_ref().cloned());
        char_block_ids.push(bid);
      }
    }
    (text_content, char_block_ids)
  }

  fn apply_open_close_tag(tag_buf: &str, stack: &mut Vec<Option<String>>) {
    let t = tag_buf.trim();
    if t.len() < 2 || !t.starts_with('<') || !t.ends_with('>') {
      return;
    }
    let inner = t[1..t.len() - 1].trim();
    if inner.starts_with("!--") || inner.to_ascii_lowercase().starts_with("!doctype") {
      return;
    }
    ...
    let bid = DATA_BLOCK_ID_ATTR
      .captures(inner)
      .and_then(|c| c.get(1))
      .map(|m| m.as_str().to_string());
    let first_tok = inner.split_whitespace().next().unwrap_or("");
    let mut tag_name = first_tok.to_ascii_lowercase();
    if let Some(i) = tag_name.find(|c: char| c == '/' || c.is_whitespace()) {
      tag_name.truncate(i);
    }
    tag_name = tag_name.trim_end_matches('/').to_string();
    const VOID: &[&str] = &[
    ...
    let is_void =
      trim_inner.ends_with("/>") || trim_inner.ends_with('/') || VOID.contains(&tag_name.as_str());
    stack.push(bid);
    if is_void {
      stack.pop();
    }
  ...
  fn char_run_start(ids: &[Option<String>], idx: usize) -> usize {
    let id = &ids[idx];
    let mut i = idx;
  ...
  /// 将 strip 后纯文本的半开区间 `[strip_start, strip_end)`（与 `Diff.old_text_char_range` 一致）映射为 canonical 四元组。
  pub fn map_stripped_char_range_to_block_canonical(
    html: &str,
    strip_start: usize,
    strip_end: usize,
  ) -> Option<(String, usize, String, usize)> {
    let (_text, ids) = Self::build_stripped_text_with_block_ids(html);
    let n = ids.len();
    if strip_start >= strip_end || strip_end > n {
      return None;
    }
    ...
    let sbid = ids[strip_start].as_ref()?.clone();
    let ebid = ids[strip_end - 1].as_ref()?.clone();

    if sbid == ebid {
      let rs_s = Self::char_run_start(&ids, strip_start);
      let rs_e = Self::char_run_start(&ids, strip_end - 1);
      if rs_s != rs_e {
        return None;
      }
      for i in strip_start..strip_end {
        if ids[i].as_deref() != Some(sbid.as_str()) {
          return None;
        }
      }
      return Some((sbid, strip_start - rs_s, ebid, strip_end - rs_s));
    }

    let start_run = Self::char_run_start(&ids, strip_start);
    let end_run = Self::char_run_start(&ids, strip_end - 1);
    Some((sbid, strip_start - start_run, ebid, strip_end - end_run))
  }
  ...
  pub fn calculate_diff(&self, old_content: &str, new_content: &str) -> Result<Vec<Diff>, String> {
    // 检查内容大小（防止大文件导致性能问题）
      ...
      match op.tag() {
        ...
        DiffTag::Delete => {
          ...
          diffs.push(Diff {
            ...
            old_text_char_range: Some((text_start, text_end)),
        ...
        DiffTag::Insert => {
          ...
          let new_end_line =
            Self::char_pos_to_line(new_content, new_html_end_char.saturating_sub(1));
        ...
        DiffTag::Replace => {
          ...
          // 使用字符迭代器安全提取，避免多字节字符问题
          let original_code =
            Self::extract_text_safe(&old_map.text_content, old_text_start, old_text_end);
          ...
          // 使用字符迭代器安全提取，避免多字节字符问题
          let new_code =
            Self::extract_text_safe(&new_map.text_content, new_text_start, new_text_end);
          ...
          diffs.push(Diff {
            ...
            element_type: None,       // 将在 tool_service 中设置
            element_identifier: None, // 将在 tool_service 中设置
            old_text_char_range: Some((old_text_start, old_text_end)),
            occurrence_index: None,
          });
    ...
    while i < diffs.len() {
      if i < diffs.len() - 1 {
        ...
        {
          // 检查是否在同一行或行号连续
          let same_or_adjacent_line = current.original_start_line == next.original_start_line  // 同一行
          ...
          // 检查在新内容中的位置是否相同或连续
          let same_or_adjacent_new_line = current.end_line == next.start_line  // 新内容中同一行
          ...
          if same_or_adjacent_line && same_or_adjacent_new_line {
            ...
            merged_diffs.push(Diff {
              ...
              old_text_char_range: current.old_text_char_range,
    ...
    // 第二次合并：仅在「strip 后旧纯文本区间首尾相接」时合并多个 Edit，避免把多处 replace 拼成
    // 正文中不存在的子串（例如 replace all `world` 两处 → 误合并为 `worldworld`），导致 canonical 映射失败。
    ...
    while i < diffs.len() {
      ...
      let mut merged_old_range = current.old_text_char_range;
      let mut j = i + 1;

      while j < diffs.len() {
        let next = &diffs[j];
        let contiguous_old = match (merged_old_range, next.old_text_char_range) {
          (Some((start, end)), Some((ns, ne))) if end == ns => {
            merged_old_range = Some((start, ne));
            true
          }
          _ => false,
        };
      ...
      if j > i + 1 {
        final_diffs.push(Diff {
          ...
          element_type: current.element_type.clone(),
          element_identifier: current.element_identifier.clone(),
          old_text_char_range: merged_old_range,
          occurrence_index: current.occurrence_index,
        });
        i = j;
      } else {
