...
impl TextBoxService {
  ...
  fn extract_vml_textboxes(xml: &str) -> Result<Vec<TextBoxInfo>, String> {
    ...
    for (index, cap) in vml_pattern.captures_iter(xml).enumerate() {
      ...
      let shape_match = Regex::new(r#"<v:shape[^>]*style="([^"]*)""#).ok();
  ...
  fn parse_drawingml_position(anchor_xml: &str) -> Result<(f64, f64, f64, f64), String> {
    ...
    let left = pos_h_pattern
      .captures(anchor_xml)
      .and_then(|c| c.get(1))
      .and_then(|m| m.as_str().parse::<f64>().ok())
      .unwrap_or(0.0);

    let top = pos_v_pattern
      .captures(anchor_xml)
      .and_then(|c| c.get(1))
      .and_then(|m| m.as_str().parse::<f64>().ok())
      .unwrap_or(0.0);
  ...
  /// 解析 DrawingML 样式信息
  fn parse_drawingml_style(
    anchor_xml: &str,
  ) -> Result<(Option<TextBoxBorder>, Option<TextBoxFill>, Option<f64>), String> {
    use regex::Regex;
    ...
    let rotation_pattern =
      Regex::new(r#"rot="(\d+)""#).map_err(|e| format!("正则表达式错误: {}", e))?;
  ...
  fn convert_textbox_content_to_html(content_xml: &str) -> Result<String, String> {
    ...
    let text_pattern =
      Regex::new(r#"<w:t[^>]*>([^<]*)</w:t>"#).map_err(|e| format!("正则表达式错误: {}", e))?;
  ...
  pub fn textbox_to_html(textbox: &TextBoxInfo) -> String {
    ...
    let emu_to_px = |emu: f64| -> f64 { (emu / 914400.0) * 96.0 };
