...
impl TaskProgressAnalyzer {
  /// 分析任务完成度
  pub fn analyze(tool_results: &[(String, String, ToolResult)]) -> TaskProgress {
    use std::collections::HashSet;
    ...
    let has_create_folder = tool_results
      .iter()
      .any(|(_, name, _)| name == "create_folder");
    let has_delete_file = tool_results
      .iter()
      .any(|(_, name, _)| name == "delete_file");
    let has_edit_doc = tool_results
      .iter()
      .any(|(_, name, result)| name == "edit_current_editor_document" && result.success);
    let is_file_move_task = has_create_folder || has_move_file;
    let is_check_all_folders_task =
      has_list_files && !is_file_move_task && !has_delete_file && list_files_call_count > 1;
    let is_delete_task = has_delete_file;

    // 判断任务类型（在进度计算之前确定，以便进度段按类型分支）
    let task_type = if has_edit_doc {
      let has_update_file = tool_results
        .iter()
        .any(|(_, name, result)| name == "update_file" && result.success);
      if has_update_file {
        TaskType::MultiDocumentEdit
      } else {
        TaskType::DocumentEdit
      }
    } else if is_check_all_folders_task {
    ...
    if task_type == TaskType::DocumentEdit || task_type == TaskType::MultiDocumentEdit {
      // 文档编辑任务：永远不主动判完成，也不判未完成
      // 由模型自然停止工具调用来结束循环
      // is_completed 和 is_incomplete 保持初始值 false
      let edit_call_count = tool_results
        .iter()
        .filter(|(_, name, result)| name == "edit_current_editor_document" && result.success)
        .count();
      progress_hint.push_str(&format!(
        "Document edit operations completed so far: {}",
        edit_call_count
      ));
    } else if is_check_all_folders_task {
      // 检查所有文件夹任务
