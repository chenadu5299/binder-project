...
impl LibreOfficeService {
    ...
    fn write_font_substitution_config(&self) -> Result<(), String> {
        ...
        eprintln!("🔤 [字体调试] 写入字体配置: 内容长度={} 字节, 默认CJK={}, 默认Latin={}", fontsubst_content.len(), default_cjk_font, default_latin_font);
        eprintln!("🔤 [字体调试] 配置内容片段(前200字符): {}...", content_preview);
        ...
        for config_path in &config_paths {
            ...
            eprintln!("🔤 [字体调试] 写入后校验: 路径={:?}, 存在={}, 大小={} 字节", fontsubst_file, exists_after, size_after);
        }
    ...
    fn generate_fontsubst_content(cjk_font: &str, latin_font: &str) -> String {
        ...
        let cjk_fonts: &[&str] = &[
            ...
            "仿宋", "FangSong", "仿宋_GB2312", "楷体_GB2312",
            "华文黑体", "华文楷体", "华文宋体", "华文仿宋", "华文中宋",
            "STHeiti", "STKaiti", "STSong", "STFangsong", "STXihei",
            "Hiragino Sans GB", "冬青黑体", "苹方", "PingFang TC", "PingFang HK",
            "隶书", "幼圆", "新宋体", "NSimSun", "方正兰亭黑", "思源黑体", "Source Han Sans SC",
        ];
        let latin_fonts: &[&str] = &[
...
{}
    ...
    pub fn convert_docx_to_pdf(&self, docx_path: &Path) -> Result<PathBuf, String> {
        ...
        if !output.status.success() {
            eprintln!("❌ LibreOffice 命令执行失败，退出码: {:?}", output.status.code());
                ...
                if let Ok(entry) = entry {
                    ...
                    if let Ok(metadata) = std::fs::metadata(&path) {
                        ...
                        eprintln!("   - {:?} (大小: {} 字节, 类型: {})", 
                            ...
                            metadata.len(),
                            if metadata.is_file() { "文件" } else { "目录" });
                    }
                }
            }
            if file_count == 0 {
                    ...
                    for entry in entries {
                        if let Ok(entry) = entry {
                            ...
                            if let Ok(metadata) = std::fs::metadata(&path) {
                                ...
                                eprintln!("  - {:?} (无法读取元数据)", path.file_name().unwrap_or_default());
                            }
                        }
                    }
                    if !has_files {
    ...
    pub fn convert_excel_to_pdf(&self, excel_path: &Path) -> Result<PathBuf, String> {
        ...
        if !output.status.success() {
            eprintln!("❌ LibreOffice 命令执行失败，退出码: {:?}", output.status.code());
            return Err(format!("LibreOffice 转换失败: {}", if !stderr.is_empty() { stderr.to_string() } else { stdout.to_string() }));
        }
    ...
    pub fn convert_presentation_to_pdf(&self, presentation_path: &Path) -> Result<PathBuf, String> {
        ...
        // 3. 执行转换（每次转换前确保字体配置已写入）
        eprintln!("🔤 [字体调试] 转换类型=演示文稿(PPT) 输入={:?}", presentation_path);
        let _ = self.write_font_substitution_config();
    ...
    fn build_libreoffice_command(&self, libreoffice_path: &Path) -> Result<Command, String> {
            ...
            .and_then(|p| p.parent())  // Contents
        {
            ...
            let fontsubst_4_path = user_config_dir.join("4").join("user").join("config").join("fontsubst.xcu");
            let (cjk, latin) = Self::get_default_fonts();
            eprintln!("🔤 [预览模式] 字体配置: UserInstallation={}, 字体文件={:?}, 默认 中文={} 英文={}", installation_url, fontsubst_path, cjk, latin);
            let exist1 = fontsubst_path.exists();
            let size1 = fs::metadata(&fontsubst_path).map(|m| m.len()).unwrap_or(0);
            let exist2 = fontsubst_4_path.exists();
            let size2 = fs::metadata(&fontsubst_4_path).map(|m| m.len()).unwrap_or(0);
    ...
    fn find_generated_pdf(&self, output_dir: &Path, source_path: &Path) -> Result<PathBuf, String> {
        ...
        eprintln!("🔍 [预览] 开始查找，最多重试 {} 次，每次等待 {}ms", MAX_RETRIES + 1, RETRY_DELAY_MS);
        ...
        for attempt in 0..=MAX_RETRIES {
            ...
            if !pdf_files.is_empty() {
                ...
                eprintln!("✅ [预览] 选择最新的 PDF 文件: {} (大小: {} 字节)", file_name, size);
                return Ok(path.clone());
            }
            
            // 如果没找到文件且不是最后一次尝试，等待后重试
            if attempt < MAX_RETRIES {
                eprintln!("⏳ [预览] 未找到 PDF 文件，等待 {}ms 后重试... (尝试 {}/{})", 
        ...
        Err(format!("PDF 文件未生成在输出目录: {:?} (已重试 {} 次)", output_dir, MAX_RETRIES + 1))
    }
