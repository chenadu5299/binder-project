...
pub enum ConversationError {
  ...
  /// JSON解析错误
  JSONParseError { message: String, arguments: String },
...
impl ExceptionHandler {
  ...
  fn find_alternative(&self, error: &ConversationError, _context: &ErrorContext) -> Option<String> {
    match error {
      ...
      ConversationError::ToolCallError {
        tool_name,
        error_type,
        ..
      } => match error_type {
        ToolCallErrorType::FileNotFound => Some(format!(
          "提示AI文件路径可能错误，建议检查路径: {}",
          tool_name
        )),
        ToolCallErrorType::ParameterError => Some(format!("尝试修复工具调用参数: {}", tool_name)),
        _ => None,
      },
      _ => None,
    }
  ...
  fn generate_user_message(&self, error: &ConversationError) -> String {
    match error {
      ...
      ConversationError::APIError {
        message,
        error_type,
        ..
      } => match error_type {
        APIErrorType::InvalidAPIKey => "API Key 无效，请检查配置".to_string(),
        APIErrorType::QuotaExceeded => "API 配额已用完，请升级或切换模型".to_string(),
      ...
      ConversationError::ContextTooLong {
        ...
        format!(
          "对话历史过长（{} tokens，限制 {} tokens），请开启新对话或减少引用内容",
          current_tokens, max_tokens
        )
      }
      ConversationError::FileTooLarge {
        path,
        size,
        max_size,
      } => {
        format!(
          "文件过大（{}，{} bytes，限制 {} bytes），无法处理",
          path, size, max_size
        )
      }
    }
  ...
  fn generate_suggestions(
    ...
    match error {
      ...
      ConversationError::APIError { error_type, .. } => match error_type {
        APIErrorType::InvalidAPIKey => {
          vec![
            "检查 API Key 配置".to_string(),
            "重新配置 API Key".to_string(),
          ]
        }
