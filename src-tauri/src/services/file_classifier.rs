...
只返回 JSON，不要其他文字。"#,
    ...
    let messages = vec![crate::services::ai_providers::ChatMessage {
      role: "user".to_string(),
      content: Some(prompt),
      tool_call_id: None,
      name: None,
  ...
  pub async fn classify_files(
    ...
    for file in files {
      ...
      match Self::extract_text_content(&file) {
        Ok(content) => {
          // 使用 AI 分类
          match Self::classify_file_with_ai(provider.clone(), &file, &content, &existing_folders)
            .await
          {
            Ok(classification) => {
