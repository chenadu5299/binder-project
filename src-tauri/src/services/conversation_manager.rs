...
pub enum ConversationState {
  ...
  /// 等待AI回复
  WaitingAIResponse { message_id: String },
...
impl ConversationManager {
  ...
  pub fn update_streaming_text(&mut self, tab_id: &str, text: &str) {
    if let Some(ConversationState::StreamingResponse {
      message_id,
      accumulated_text,
  ...
  pub fn update_tool_call_status(&mut self, tab_id: &str, status: ToolCallStatus) {
    if let Some(ConversationState::ToolCalling {
      message_id,
      tool_call_id,
      tool_name,
      ..
  ...
  pub fn get_history(&self, tab_id: &str) -> Vec<crate::services::ai_providers::ChatMessage> {
    self.histories.get(tab_id).cloned().unwrap_or_default()
  }
