...
pub fn ensure_workspace_memory_schema(conn: &Connection) -> Result<(), String> {
  conn
    .execute_batch(WORKSPACE_MEMORY_DDL)
...
impl Default for ExtractionConfig {
  fn default() -> Self {
    ExtractionConfig {
      model: std::env::var("BINDER_MEMORY_MODEL").unwrap_or_else(|_| "deepseek-chat".to_string()),
...
impl MemoryService {
  ...
  pub fn new(workspace_path: &Path) -> Result<Self, String> {
    ...
    let conn =
      rusqlite::Connection::open(&db_path).map_err(|e| format!("打开 workspace.db 失败: {}", e))?;
  ...
  pub async fn upsert_tab_memories_with_judge(
    ...
    for item in &items {
      ...
      eprintln!(
        "[memory] judge: {:?} for entity={}",
        action, item.entity_name
      );
  ...
  pub async fn upsert_content_memories_with_judge(
    ...
    for item in &items {
      ...
      let action = llm_judge_action(provider, item, &similar).await;
      eprintln!(
        "[memory] content judge: {:?} for entity={}",
        action, item.entity_name
      );
  ...
  pub async fn get_last_content_extraction_time(&self, file_path: &str) -> Option<i64> {
    let db = self.db.clone();
    let file_path = file_path.to_string();
    tokio::task::spawn_blocking(move || {
            let conn = db.lock().ok()?;
  ...
  pub async fn rebind_content_memories_for_path(
    ...
    let db = self.db.clone();
    let old_norm = old_path
      .replace('\\', "/")
      .trim_end_matches('/')
      .to_string();
    let new_norm = new_path
      .replace('\\', "/")
      .trim_end_matches('/')
      .to_string();
    tokio::task::spawn_blocking(move || {
  ...
  pub async fn expire_layer(&self, layer: &str) -> Result<u64, MemoryError> {
    ...
    tokio::task::spawn_blocking(move || {
      ...
      eprintln!(
        "[memory] P2: batch expired layer={}, count={}",
        layer, count
      );
      Ok(count as u64)
    })
...
fn execute_fts_search(
  ...
  let stale_ids: Vec<String> = rows
    .iter()
    .filter(|(item, _)| item.freshness_status == "fresh" && now - item.updated_at > seven_days)
...
fn cascade_supersede_related(
  ...
  let mut count = 0u64;
  for sim in &similar {
    if sim.id == new_item_id {
      continue;
    } // 跳过刚写入的新条目
    if sim.freshness_status == "expired" || sim.freshness_status == "superseded" {
      continue;
    }
    // 仅 supersede 置信度 >= 0.5 的相关条目（避免误伤低相关记忆）
    if sim.confidence < 0.5 {
      continue;
    }
    conn.execute(
      "UPDATE memory_items SET freshness_status = 'superseded', updated_at = ?1 WHERE id = ?2",
    ...
    count += 1;
    eprintln!(
      "[memory] evolution: cascade superseded id={} (related to updated {})",
      sim.id, new_item_id
    );
  }
  Ok(count)
...
fn build_scope_ids(params: &SearchMemoriesParams) -> Vec<String> {
  ...
  match &params.scope {
    MemorySearchScope::Tab => {
      if let Some(ref t) = params.tab_id {
        ids.push(t.clone());
      }
    }
    MemorySearchScope::Content | MemorySearchScope::WorkspaceLongTerm => {
      if let Some(ref w) = params.workspace_path {
        ids.push(w.clone());
      }
    }
...
pub fn find_similar_memories(
  ...
  let like_pat = format!(
    "%{}%",
    candidate.entity_name.chars().take(20).collect::<String>()
  );
  let mut stmt = match conn.prepare(sql) {
    Ok(s) => s,
    Err(_) => return vec![],
...
{{"action": "ADD"|"UPDATE"|"DELETE"|"NOOP", "target_id": "相关记忆ID或null"}}"#,
    candidate.entity_name, candidate.entity_type, candidate.content, similar_text,
  )
}
...
pub async fn llm_judge_action(
  ...
  let response = match provider.chat_simple(&prompt, 100).await {
    ...
    Err(e) => {
      eprintln!(
        "[memory] llm_judge_action: AI call failed: {:?}, defaulting to ADD",
        e
      );
  ...
  let v: serde_json::Value = match serde_json::from_str(json_str) {
    ...
    Err(_) => {
      eprintln!(
        "[memory] llm_judge_action: parse failed, defaulting to ADD. response={}",
        response
      );
      return JudgeAction::Add;
    }
...
fn trim_memory_to_budget(lines: &mut Vec<String>, header: Option<&str>) {
  ...
  loop {
    let current_len: usize =
      lines.iter().map(|l| l.len() + 1).sum::<usize>() + wrapper_overhead + header_len;
...
pub fn format_memory_for_injection(items: &[MemorySearchResult]) -> String {
  ...
  let anchor_header = if items.len() > 5 {
    Some(
      "以下内容来自历史对话或文档分析的记忆提炼，供参考，不代表当前现场事实，请结合实际情况判断。",
    )
  } else {
    None
  };
  ...
  for r in items {
    ...
    let body = if r.item.summary.is_empty() {
      &r.item.content
    } else {
      &r.item.summary
    };
    let stale_mark = if r.item.freshness_status == "stale" {
      " [旧]"
    } else {
      ""
    };
    lines.push(format!(
      "{}{} {}: {}",
      label, stale_mark, r.item.entity_name, body
    ));
  }
...
pub async fn memory_generation_task_tab(
  ...
  if let Err(e) = svc
    .upsert_tab_memories_with_judge(&tab_id, candidates, &provider)
    .await
  {
...
pub async fn memory_generation_task_content(
  ...
  let svc = match MemoryService::new(&workspace_path) {
    ...
    Err(e) => {
      eprintln!(
        "[memory] content extraction: MemoryService init failed: {}",
        e
      );
      return;
    }
  ...
  if let Err(e) = svc
    .upsert_content_memories_with_judge(&file_path, candidates, &provider)
    .await
  {
    eprintln!(
      "[memory] content extraction: upsert_with_judge failed: {:?}",
      e
    );
  } else {
...
pub async fn on_tab_deleted(
  ...
  if user_message_count >= 5 {
    let Some(provider) = provider else {
      eprintln!(
        "[memory] on_tab_deleted: provider unavailable, skip promotion for tab={}",
        tab_id
      );
      ...
      let _ = tokio::task::spawn_blocking(move || {
        ...
        eprintln!(
          "[memory] on_tab_deleted: expired all tab memories for tab={}",
          tab_id_clone
        );
      })
      .await;
...
pub fn filter_sensitive_content(text: &str) -> String {
  ...
  if total > 5 && removed * 100 / total > 80 {
    eprintln!(
      "[memory] S-04: document rejected — {}% of lines flagged as sensitive",
      removed * 100 / total
    );
    return String::new();
  }
...
pub async fn merge_with_user_memories(
  ...
  let user_results = search_user_memories(query, user_limit)
    .await
    .unwrap_or_default();
  ...
  let existing_ids: std::collections::HashSet<String> =
    merged.iter().map(|r| r.item.id.clone()).collect();
...
mod tests {
  ...
  impl TestWorkspace {
    fn new(label: &str) -> Self {
      let path =
        std::env::temp_dir().join(format!("binder-memory-{}-{}", label, uuid::Uuid::new_v4()));
  ...
  async fn tab_memory_survives_service_restart_and_active_tab_is_not_marked_orphan() {
    ...
    assert_eq!(
      response.total_found, 1,
      "tab memory should still be searchable"
    );
