...
use super::types::{
  ...
  KnowledgeEntry, KnowledgeErrorCode, KnowledgeFolder, KnowledgeProvenance, KnowledgeRetrievalMode,
  KnowledgeRetrievalStatus, KnowledgeSourceRole, KnowledgeSourceStatus, KnowledgeStageEvent,
  KnowledgeStructureMetadata, KnowledgeSyncMode, KnowledgeVerificationStatus,
...
impl KnowledgeService {
  ...
  fn run_migrations(&self) -> Result<(), String> {
    let conn = self
      .conn
      .lock()
      .map_err(|e| format!("knowledge 锁失败: {}", e))?;
    conn
      .execute(
  ...
  pub(crate) fn lock_conn(
    &self,
  ) -> Result<std::sync::MutexGuard<'_, Connection>, KnowledgeServiceError> {
  ...
  pub(crate) fn resolve_content_input(
    ...
    let effective_title = title
      ...
      .filter(|raw| !raw.is_empty())
      .or_else(|| {
        effective_source_ref.as_ref().and_then(|source| {
          PathBuf::from(source)
            .file_name()
  ...
  fn resolve_workspace_snapshot_path(
    ...
    let workspace_canonical =
      fs::canonicalize(&self.workspace_path).map_err(|e| KnowledgeServiceError {
  ...
  pub(crate) fn map_document(row: &Row<'_>) -> rusqlite::Result<KnowledgeDocument> {
    ...
    let provenance =
      serde_json::from_str::<KnowledgeProvenance>(&provenance_json).map_err(|e| {
  ...
  pub(crate) fn build_citation(
    ...
    let citation_key = match chunk_id {
      Some(chunk_id) => format!(
        "kc:{}:{}:{}:v{}",
        entry.id, document.id, chunk_id, document.version
      ),
  ...
  pub(crate) fn hydrate_entry_source_status(&self, entry: &mut KnowledgeEntry) {
    let (status, message) =
      self.resolve_source_status(entry.source_type.as_str(), entry.source_ref.as_deref());
