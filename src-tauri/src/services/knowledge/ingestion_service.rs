...
impl KnowledgeService {
    pub fn ingest_document(
        ...
        let metadata_json = Self::serialize_document_metadata(
            &asset_kind,
            request.metadata,
            structure_metadata.clone(),
        )?;
        let provenance_json = serde_json::to_string(&provenance).map_err(|e| KnowledgeServiceError {
            code: KnowledgeErrorCode::PersistenceFailed,
            message: format!("序列化 provenance 失败: {}", e),
        })?;

        let mut conn = self.lock_conn()?;
        let tx = conn.transaction().map_err(|e| self.db_error(KnowledgeErrorCode::PersistenceFailed, e))?;

        Self::insert_stage_event(&tx, "entry", &entry_id, "validate", "ok", None, None, false)?;
        Self::insert_stage_event(&tx, "document", &document_id, "validate", "ok", None, None, false)?;
        ...
        Self::insert_stage_event(&tx, "document", &document_id, "persist", "ok", None, None, false)?;
        Self::insert_stage_event(&tx, "document", &document_id, "parse", "ok", None, None, false)?;
        ...
        Self::insert_stage_event(&tx, "document", &document_id, "chunk", "ok", None, None, false)?;
        Self::insert_stage_event(&tx, "document", &document_id, "index", "ok", None, None, false)?;
        ...
        tx.execute(
            "UPDATE knowledge_entries
             SET active_document_id = ?2,
                 ...
                 updated_at = ?4
             WHERE id = ?1",
            params![entry_id, document_id, Self::eligible_retrieval_status(), now],
        )
        .map_err(|e| KnowledgeServiceError {
            code: KnowledgeErrorCode::PersistenceFailed,
            message: format!("更新 knowledge entry active version 失败: {}", e),
        })?;
        Self::insert_stage_event(&tx, "document", &document_id, "ready", "ok", None, None, false)?;

        tx.commit().map_err(|e| self.db_error(KnowledgeErrorCode::PersistenceFailed, e))?;
        drop(conn);
                ...
                tx.execute(
                    "UPDATE knowledge_documents
                     SET state = ?2,
                         ...
                         updated_at = ?3
                     WHERE id = ?1",
                    params![active_document_id, KnowledgeDocumentState::Superseded.as_str(), now],
                )
                .map_err(|e| KnowledgeServiceError {
    ...
    pub fn upsert_workspace_snapshot(
        ...
        let effective_source_ref = resolved
            .source_ref
            .as_deref()
            .unwrap_or(source_path);

        if let Some(existing_entry) = self.find_workspace_snapshot_entry_by_source_ref(
            &knowledge_base.id,
    ...
    fn resolve_structure_metadata(
        ...
        match asset_kind {
            ...
            KnowledgeAssetKind::StructureAsset => {
                let metadata = structure_metadata.or(existing_structure_metadata).ok_or_else(|| {
    ...
    fn validate_structure_metadata(
        ...
        if metadata.document_form.trim().is_empty()
            ...
            || metadata.slot_hints.iter().any(|value| value.trim().is_empty())
        {
            return Err(KnowledgeServiceError {
                code: KnowledgeErrorCode::InvalidInput,
