...
impl KnowledgeService {
    pub fn delete_entry(
        ...
        tx.commit().map_err(|e| self.db_error(KnowledgeErrorCode::DeleteFailed, e))?;
        drop(conn);
    ...
    pub fn move_entry(
        ...
        if let (Some(source_ref), Some(active_document_id)) = (
            new_source_ref.as_ref(),
            entry.active_document_id.as_deref(),
        ) {
        ...
        Self::insert_stage_event(&tx, "entry", &entry.id, "metadata_reindex", "ok", None, None, false)?;
    ...
    pub fn update_entry_policy(
        ...
        let _ = self.insert_stage_event_direct("entry", &entry.id, "policy_update", "ok", None, None, false);
