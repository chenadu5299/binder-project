...
impl KnowledgeService {
  pub fn list_knowledge_entries(
    ...
    for row in rows {
      ...
      items.push(KnowledgeEntryListItem {
        ...
        preview,
        citation,
        structure_metadata: document
          .as_ref()
          .and_then(|doc| doc.structure_metadata.clone()),
      });
    }
  ...
  pub fn query_knowledge_base(
    ...
    let chunk_hits: Vec<KnowledgeChunkHit> =
  ...
  fn fetch_query_rows(
    ...
    if matches!(intent, KnowledgeQueryIntent::Augmentation) {
      results.retain(|row| {
        row.entry.source_status == "ready"
          && KnowledgeService::supports_automatic_retrieval(&row.entry)
      });
    }
    Ok(results)
  }
...
fn score_structure_row(
  ...
  let mut score = 0.0f64;
  if let Some(target_form) = structure_document_form
  ...
  if let Some(target_purpose) = structure_purpose
    .map(|value| value.trim().to_lowercase())
    .filter(|value| !value.is_empty())
  {
  ...
  (score, metadata.section_outline_summary.clone())
}
