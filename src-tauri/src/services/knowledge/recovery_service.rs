...
impl KnowledgeService {
  pub fn rebuild_entry(
    ...
    let operation = || -> Result<(), KnowledgeServiceError> {
      ...
      for row in rows {
        existing_chunks
          .push(row.map_err(|e| self.db_error(KnowledgeErrorCode::PersistenceFailed, e))?);
      }

      for chunk in &chunks {
        if let Some(existing) = existing_chunks
          .iter()
          .find(|row| row.chunk_index == chunk.chunk_index)
        {
          tx.execute(
      ...
      for existing in existing_chunks
        .iter()
        .filter(|row| row.chunk_index >= chunks.len())
      {
        tx.execute(
          "UPDATE knowledge_chunks
                     SET state = 'deleted',
