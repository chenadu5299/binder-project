...
mod tests {
  ...
  fn p0_roundtrip_ingest_query_replace_delete() {
    ...
    assert_eq!(
      v1_citation.provenance.source_ref.as_deref(),
      Some("notes.md")
    );
  ...
  fn p1_workspace_snapshot_upsert_uses_explicit_replace_chain() {
    ...
    assert_eq!(
      second.document.as_ref().expect("second document").version,
      2
    );
  ...
  fn p2_replace_preserves_historical_citation_and_deleted_status() {
    ...
    assert_eq!(
      superseded_query.chunk_hits[0].citation.citation_key,
      v1_citation_key
    );
  ...
  fn p2_rebuild_and_retry_preserve_active_version() {
      ...
      .ingest_document(KnowledgeIngestRequest {
        ...
        content: Some(
          (0..18)
            .map(|idx| {
              format!(
                "Section {} rebuild preserves active version and chunk identity.",
                idx
              )
            })
            .collect::<Vec<_>>()
    ...
    assert_eq!(
      rebuild.document.as_ref().expect("doc").id,
      active_document.id
    );
    assert_eq!(
      rebuild.document.as_ref().expect("doc").version,
      active_document.version
    );
    ...
    assert_eq!(
      rebuilt_query.document_hits[0].document.id,
      active_document.id
    );
    assert_eq!(
      rebuilt_query.chunk_hits[0].citation.citation_key,
      first_citation_key
    );
    ...
    assert_eq!(
      retried.document.as_ref().expect("doc").version,
      active_document.version
    );
  ...
  fn p3_source_defaults_and_policy_controls_gate_automatic_retrieval() {
    ...
    fs::write(
      &source_path,
      "Binder policy source content for automatic retrieval.",
    )
  ...
  fn p3_hybrid_query_metadata_and_root_scope_query_work() {
    ...
    service
      .ingest_document(KnowledgeIngestRequest {
        ...
        content: Some(
          "Matrix section lists workspace auto and explicit only policy states.".to_string(),
        ),
        ..Default::default()
      })
    ...
    assert_eq!(
      hybrid_query.entry_hits[0].entry.title,
      "Binder Policy Matrix"
    );
